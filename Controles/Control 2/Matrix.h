...
public:
	...
	Matrix operator*(const Matrix &v2) const{
		...
		for(int i=0;i<4;i++){
			for(int j=0;j<4;j++){
				float acum=0.0f;
				for(int k=0;k<4;k++){
					acum += matrix[i*4 + k] * v2.matrix[k*4 + j];
				}
	...
	void translate_norm(float first_val,float second_val,float third_val){
		...
		tmp.matrix[3] = first_val;
		tmp.matrix[7] = second_val;
		tmp.matrix[11] = third_val;
		
		tmp=(*this)*tmp;
		matrix=tmp.matrix;
		std::cout << "POS TRASLACION" << std::endl;
		PrintMatrix();
	}
	...
	void UpdateView(char tpe,float first_val,float second_val=0.0f,float third_val=0.0f,char axis='z'){
		...
		switch(type){
			...
			case 'd':{
				...
				std::cout << "Matriz ANTES:" << std::endl;
				PrintMatrix();
				...
				std::cout << "Matriz DESPUES:" << std::endl;
				PrintMatrix();
				break;
			}
			case 'f':{
